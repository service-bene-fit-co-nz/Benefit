...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!programmeHabitId || !habitDate) {
      return NextResponse.json(
        { error: "Programme habit ID and completion date are required" },
        { status: 400 }
      );
    }
    ...
    if (!programmeHabit) {
      return NextResponse.json(
        { error: "Programme habit not found" },
        { status: 404 }
      );
    }
    ...
    if (programmeHabit.programme.enrolments.length === 0) {
      return NextResponse.json(
        { error: "Not enrolled in programme" },
        { status: 403 }
      );
    }
    ...
    const requiredPerDay = Math.max(
      1,
      programmeHabitDetails?.[
        currentDayFrequencyKey as keyof typeof programmeHabitDetails
      ] ?? 1
    );
    ...
    try {
      ...
      if (existingCompletion) {
        const nextTimesDone = clamp(
          (existingCompletion.timesDone ?? 0) +
            (typeof delta === "number" ? delta : completed ? 1 : -1),
          0,
          MAX_PER_DAY
        );
        ...
        const initialTimesDone = clamp(
          typeof delta === "number" ? Math.max(delta, 0) : completed ? 1 : 0,
          0,
          MAX_PER_DAY
        );
    ...
    return NextResponse.json({
      ...completion,
      requiredPerDay:
        completion.programmeHabit?.[
          currentDayFrequencyKey as keyof typeof completion.programmeHabit
        ] ?? 1,
    });
  } catch (error: any) {
