...
export async function getClientActivities(
  ...
  if (!client || !client.settings) {
    ...
    return { success: false, message: "Client or client settings not found.", code: "CLIENT_NOT_FOUND" };
  }

  if (typeof client.settings !== "object" || client.settings === null) {
    console.error("Client settings are not a valid object.");
    return { success: false, message: "Client settings are not a valid object.", code: "INVALID_CLIENT_SETTINGS" };
  }
  ...
  if (!fitbitClientSettings || fitbitClientSettings.length === 0) {
    ...
    return { success: false, message: "Fitbit is not connected for this client.", code: "FITBIT_NOT_CONNECTED" };
  }
  ...
  if (!accessToken || !refreshToken || !expiresAt || !fitbitUserId) {
    ...
    return { success: false, message: "Missing Fitbit connection details. Please re-connect Fitbit.", code: "MISSING_FITBIT_DETAILS" };
  }
  ...
  if (isTokenExpired) {
    ...
    try {
      ...
      if (!clientIdEnv || !clientSecretEnv) {
        return { success: false, message: "Fitbit client environment variables not defined.", code: "MISSING_ENV_VARS" };
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 && newTokens.errors && newTokens.errors.some((err: any) => err.errorType === 'invalid_grant')) {
          console.error("Invalid refresh token detected. Clearing Fitbit tokens for client:", clientId);
          // Clear Fitbit tokens from the client's settings
          const updatedFitbitProperties = fitbitConfig?.properties.filter(
            (p) => p.name !== "accessToken" && p.name !== "refreshToken" && p.name !== "expiresAt"
          ) || [];
          ...
          return { success: false, message: "Fitbit connection requires re-authorization.", code: "FITBIT_REAUTH_REQUIRED" };
        }
        return { success: false, message: "Failed to refresh Fitbit token.", code: "FITBIT_REFRESH_FAILED" }; // For other refresh failures
      }
      ...
      if (
        ...
        return { success: false, message: "Fitbit token refresh response incomplete.", code: "FITBIT_REFRESH_INCOMPLETE" };
      }
      ...
      return { success: false, message: "Error refreshing Fitbit token.", code: "FITBIT_REFRESH_ERROR" };
    }
  }
  ...
  try {
    ...
    return { success: false, message: "Error fetching Fitbit activities.", code: "FITBIT_API_ERROR" };
  }

  if (!activityLogListResponse.data) {
    console.error("No data received from Fitbit activity log list API.");
    return { success: false, message: "No data received from Fitbit activities.", code: "FITBIT_API_NO_DATA" };
  }
  ...
  return { success: true, data: activities.sort((a, b) => {
...
export async function getFitbitConnectionStatus(): Promise<ActionResult<{ status: string; message: string; }>> {
  try {
    ...
    if (!client) {
      return { success: false, message: "Client not found.", code: "CLIENT_NOT_FOUND" };
    }
    ...
    if (result.success) {
      return { success: true, data: { status: "connected", message: "Connected correctly to Fitbit." } };
    } else {
      if (result.code === "FITBIT_REAUTH_REQUIRED") {
        return { success: true, data: { status: "reauth_required", message: "Fitbit connection requires re-authorization." } };
      } else if (result.code === "FITBIT_NOT_CONNECTED" || result.code === "MISSING_FITBIT_DETAILS") {
        return { success: true, data: { status: "not_connected", message: "Fitbit is not connected." } };
      } else {
        return { success: true, data: { status: "error", message: result.message || "An unknown error occurred with Fitbit connection." } };
      }
    }
  } catch (error: any) {
    console.error("Error in getFitbitConnectionStatus:", error);
    return { success: false, message: error.message || "An unexpected error occurred.", code: "UNEXPECTED_ERROR" };
  }
}
